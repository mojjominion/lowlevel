...
class BoardStateTests(unittest.TestCase):
    ...
    def test_ladder_effect(self):
        ...
        player_at_old_position = (
            player in self.board.state[self.start.row][self.start.col]
        )
        ...
        self.assertFalse(player_at_old_position, "Player is still at old position")
        self.assertTrue(player_at_new_position, "Player has not reached new position")
    ...
    def test_winner_effect(self):
        ...
        self.assertTrue(
            self.game._isWinner(player.getCoords()),
            f"{player.name} has not reached end",
        )
        self.assertTrue(
            player.name in self.game.winners, f"{player.name} is not in winners"
        )

    def test_coinciding_tunnels(self):
        for _ in range(5):  # 225 cells - 20 * 10 = 200 pipe points
            self.board.addPipes(self.game.__generatePipes())
